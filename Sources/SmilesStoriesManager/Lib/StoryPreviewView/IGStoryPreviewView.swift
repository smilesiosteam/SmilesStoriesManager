...
public enum IGLayoutType {
    ...
    var animator: LayoutAttributesAnimator {
        switch self {
        case .cubic:return CubeAttributesAnimator(perspective: -1/100, totalAngle: .pi/24)
        }
    }
}
...
class IGStoryPreviewView: UIView {
    ...
    private func installLayoutConstraints(){
        ...
        NSLayoutConstraint.activate([
            igLeadingAnchor.constraint(equalTo: snapsCollectionView.igLeadingAnchor),
            igTopAnchor.constraint(equalTo: snapsCollectionView.igTopAnchor),
            snapsCollectionView.igTrailingAnchor.constraint(equalTo: igTrailingAnchor),
