 ...
 */
public class IGStoryPreviewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public override func viewWillAppear(_ animated: Bool) {
        ...
        if UIDevice.current.userInterfaceIdiom == .phone {
            StoriesUtilities.lockOrientation(.portrait, andRotateTo: .portrait)
        }
        if !executeOnce {
    ...
    public override func viewWillDisappear(_ animated: Bool) {
        ...
        if UIDevice.current.userInterfaceIdiom == .phone {
            ...
            StoriesUtilities.lockOrientation(.all)
        }
    }
    ...
    init?(layout:IGLayoutType = .cubic,stories: [Story],handPickedStoryIndex: Int, handPickedSnapIndex: Int = 0) {
        self.layoutType = layout
        self.stories = stories
        guard self.stories.count > 0,handPickedStoryIndex<self.stories.count,self.stories[handPickedStoryIndex].snaps?.count ?? 0>handPickedSnapIndex else {return nil}
    ...
    public override var prefersStatusBarHidden: Bool { return false }
   ...
   private func setup_viewConstraint() {
        ...
        NSLayoutConstraint.activate([
            _view.leadingAnchor.constraint(equalTo: view.igLeadingAnchor,constant: .zero),
            _view.topAnchor.constraint(equalTo: view.igTopAnchor,constant: .zero),
            _view.trailingAnchor.constraint(equalTo: view.igTrailingAnchor,constant: .zero),
            _view.bottomAnchor.constraint(equalTo: view.igBottomAnchor,constant: .zero)
        ])
    }
    func shareButtonPressed(snapIndex:Int, storyIndex:Int){
        //overridden
    }
    func favouriteButtonPressed(snapIndex:Int, storyIndex:Int){
        //overridden
    }
    func favouriteUpdated(snapIndex:Int, storyIndex:Int){
        //overridden
    }
}
...
extension IGStoryPreviewController:UICollectionViewDataSource {
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.primaryButtonAction = { sInd in
            self.dismiss()
            self.primaryButtonAction?(sInd,self.nStoryIndex)
        }
        cell.infoButtonAction = { sInd in
            self.dismiss()
            self.infoButtonAction?(sInd,self.nStoryIndex)
        }
        cell.shareButtonAction = { sInd in
            self.shareButtonPressed(snapIndex: sInd, storyIndex: self.nStoryIndex)
        }
...
extension IGStoryPreviewController: UICollectionViewDelegate {
    ...
    public func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        ...
        if vCell.story == story_copy {
            ...
            if (vCell.story?.snaps?[vCell.story?.lastPlayedSnapIndex ?? 0])?.type ?? .unknown == .video {
...
extension IGStoryPreviewController: StoryPreviewProtocol {
    ...
    func setFavourite(wishlistResponse: WishListResponseModel, operation: Int) {
        if let snapIndex = self.currentCell?.snapIndex {
            ...
            if let updateWishlistStatus = wishlistResponse.updateWishlistStatus, updateWishlistStatus {
                isFavorite = operation == 1 ? true : false
            } else if let responseCode = wishlistResponse.responseCode, responseCode == "204" {
                isFavorite = operation == 1 ? true : false
            }
    ...
    func pause(){
        currentCell?.pauseEntireSnap()
    }
    func resume(){
        currentCell?.resumeEntireSnap()
    }
    
    func currentSelectedStory() -> Story? {
        return currentCell?.story
    }
}
